...
type RenderBreakProps = { event: Event }

export function RenderBreak({ event }: RenderBreakProps) {
  const { sponsor, description, title, menuItems } = event
  ...
  return (
    <View style={FULL_SIZE}>
      ...
      {menuItems && <Text preset="sectionHeader" tx="talkDetailsScreen.menuTitle" style={LABEL} />}
      {menuItems &&
